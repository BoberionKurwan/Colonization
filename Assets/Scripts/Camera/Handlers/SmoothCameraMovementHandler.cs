...
public class SmoothCameraMovementHandler : ICameraMovementHandler
{
    ...
    {
        _cachedCameraPosition -= new Vector3(inputDelta.x, 0, inputDelta.y) 
            * _properties.Speed;

        _properties.Pivot.position = Vector3.Lerp(_properties.Pivot.position,
            _cachedCameraPosition, Time.deltaTime / _properties.Smoothness);
    }
}